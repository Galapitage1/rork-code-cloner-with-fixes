...
function LiveInventoryScreen() {
  const { products, outlets, stockChecks, salesDeductions, productConversions, requests, inventoryStocks, updateStockCheck, saveStockCheck } = useStock();
    ...
    conversionPairs.forEach(pair => {
      ...
      dates.forEach(date => {
        ...
        if (isProductionOutlet) {
          // Production outlets: first show approved production requests (existing flow)
          ...
          // ALSO surface outstanding Prods.req values created by sales reconciliation.
          // These live on inventoryStocks.prodsReqWhole/prodsReqSlices and were
          // previously only used to auto-fill production stock checks. Showing
          // them here lets the Production kitchen see the demand generated by
          // reconciliation even before a production request is approved.
          if (date === selectedDate) {
            const invStock = inventoryStocks.find(inv => inv.productId === pair.wholeId);
            const prodsReqWhole = invStock?.prodsReqWhole || 0;
            const prodsReqSlices = invStock?.prodsReqSlices || 0;

            if (prodsReqWhole > 0 || prodsReqSlices > 0) {
              console.log(`Adding outstanding Prods.req for ${pair.wholeId} on ${date}: ${prodsReqWhole}W/${prodsReqSlices}S from reconciliation`);
              receivedWhole += prodsReqWhole;
              receivedSlices += prodsReqSlices;
            }
          }
        } else {
