...
export function StockProvider({ children, currentUser, enableReceivedAutoLoad = true }: { children: ReactNode; currentUser: { id: string; username?: string; role?: 'superadmin' | 'admin' | 'user' } | null; enableReceivedAutoLoad?: boolean }) {
  ...
  const saveStockCheck = useCallback(async (stockCheck: StockCheck, skipInventoryUpdate = false) => {
    try {
      ...
      const completedByName = (stockCheck.completedBy || '').trim();
      const isAutoGeneratedStockCheck = completedByName.length === 0 || completedByName === 'AUTO';
      if (isAutoGeneratedStockCheck && completedByName.length === 0) {
        console.log('saveStockCheck: No "Checked By" name provided, treating stock check as AUTO/system-generated');
      }
      ...
      if (!isAutoGeneratedStockCheck && createSnapshotRef.current) {
        try {
      ...
      } else if (isAutoGeneratedStockCheck) {
        console.log('saveStockCheck: Skipping live inventory snapshot for AUTO/system-generated stock check');
      }

      if (isAutoGeneratedStockCheck) {
        console.log('saveStockCheck: Skipping inventory update for AUTO/system-generated stock check');
      } else if (!skipInventoryUpdate) {
    ...
    try {
      ...
      const mutableChecks = stockChecks
        .filter(check => {
          ...
          return check.outlet === request.fromOutlet && completedBy !== '' && completedBy !== 'AUTO' && !check.deleted;
        })
